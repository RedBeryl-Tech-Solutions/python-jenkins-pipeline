...
public class License implements Runnable {
    ...
    public License(CoreProcessRunner coreProcess) {
        ...
        // Set a default lastModified value or use a constant
        lastModified = System.currentTimeMillis(); // Set to the current time

        // Log a message indicating that the license is accepted
        logger.info("License accepted without checking.");

        // You may also want to notify listeners that the license has been accepted
...
//    }
    ...
    private File getLicenseFile() {
        ...
        // Return a dummy File object to represent a valid license file path
        return licenseFile;
    }
...
//    }
    ...
    private void changeLicense(long lastModified) {
        boolean isActive = true; // Always consider the license as accepted
        licenseListeners.forEach(license -> license.changed(isActive));
        this.lastModified = lastModified;
        logger.info("License changed. Status: " + (isActive ? "Accepted" : "Not accepted"));
    }
...
//    }
    ...
    public synchronized void changeLicense(String regName, String regNumber) throws IOException {
        ...
        // Always notify that the license has been accepted
        licenseListeners.forEach(license -> license.changed(true));
    }
